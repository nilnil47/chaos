...
%     first half
    time = aa(1:0.25*vec_len,4);
    measured_voltage = aa(1:0.25*vec_len,11);
    given_voltage = aa(1:0.25*vec_len,5);
...
%     plot(time,given_voltage);
    ...
    title(append('AM Signal - ',baseFileName),FontSize=16);
...
%     second half
    time = aa(0.25*vec_len:0.5*vec_len,4);
    measured_voltage = aa(0.25*vec_len:0.5*vec_len,11);
    given_voltage = aa(0.25*vec_len:0.5*vec_len,5);
    given_voltage = smoothdata(given_voltage,"gaussian");
    [given_peaks,given_peaks_time] = findpeaks(given_voltage,time,MinPeakHeight=0.1);
    [measured_peaks,measured_peaks_time] = findpeaks(measured_voltage,time,'MinPeakDistance',0.8*mean(diff(given_peaks_time)));
...
% XY - map
    ...
    figure;
    plot(given_voltage,measured_voltage);
    hold on;
    grid on;
    title(append('Measured VS Given Voltage ',erase(baseFileName,'.csv')),FontSize=16);
    xlabel('Given Voltage [V]','FontSize',13);
    ylabel('Measured Voltage [V]','FontSize',13);
